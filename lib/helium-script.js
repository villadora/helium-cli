...
var helium = {
    start: function(pageList, callback) {
        async.waterfall([
            ...
            function(list, cb) {
                helium.getcss(list, cb);
            }
        ], function(err, result) {
            system.stderr.writeLine(JSON.stringify(result, null, 4));
            callback(err, result);
        });
    ...
    getcss: function(stylesheets, callback) {
        var p = [];
            ...
            p.push(function(cb) {
                if (!ss.body) {
                    var curl,
                        body = '';
                    if (curlScript) {
                        curl = child_process.spawn("node", [curlScript, ss.url]);
                    } else {
                        curl = child_process.spawn("curl", [ss.url]);
                    }
                    ...
                    curl.on('exit', function(code) {
                        if (code !== 0) {
                            ss.err = 'received stylesheet failed';
                            return cb(null, ss);
                        } else {
                            ss.body = body;
                            cb(null, ss);
        ...
        async.paralle(p, function(err, results) {
            callback(err, results);
        });
    },
    getcssselector: function(stylesheets, callback) {
        async.mapSeries(stylesheets, function(ss, cb) {
                if (ss.body) {
